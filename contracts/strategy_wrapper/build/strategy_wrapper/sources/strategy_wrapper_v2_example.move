...
/// This shows the pattern for version 2 with additional features
module strategy_wrapper::strategy_wrapper_v2_example {
    ...
    public struct RebalanceSettings has store {
        enabled: bool,
        threshold_bps: u64,  // basis points
    ...
    // Main struct (unchanged from v1 except version handling)
    ...
    /// Initialize auto-rebalance configuration (V2+ only)
    ...
    /// Initialize performance tracking (V2+ only)
    ...
    public fun set_custom_parameters<P>(
        ...
        assert!(strategy_cap.version >= MIN_VERSION_FOR_FEATURES, EFeatureNotAvailable);
        assert!(risk_level >= 1 && risk_level <= 10, 3); // Invalid risk level
    ...
    /// Clean up dynamic fields when ejecting (V2+ only)
