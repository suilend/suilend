...
module strategy_wrapper::strategy_wrapper_tests {
    ...
    public fun setup(
        ctx: &mut TxContext,
    ): (LendingMarket<LENDING_MARKET>, ObligationOwnerCap<LENDING_MARKET>, Clock) {
    ...
    fun test_strategy_types() {
        ...
        let btc_strategy = strategy_wrapper::create_strategy_owner_cap<LENDING_MARKET>(
            &mut lending_market2,
            2,
            scenario.ctx(),
        );
    ...
    fun test_unauthorized_relayer_borrow() {
        ...
        let (mut wrapped_cap, relayer_cap) = strategy_wrapper::convert_to_wrapped_cap<
            LENDING_MARKET,
        >(
            strategy_cap,
            BACKEND,
    ...
    fun test_double_borrow_fails() {
        ...
        // This shouldn't be reached, but if it is, clean up properly
        strategy_wrapper::return_obligation_cap(
            &mut wrapped_cap,
            obligation_cap2,
            receipt2,
            scenario.ctx(),
    ...
    fun test_convert_back_to_strategy_cap() {
        ...
        assert!(
            strategy_wrapper::get_strategy_type(&restored_strategy_cap) == original_strategy_type,
        );
