module oracles::oracles_tests {
    use oracles::{mock_pyth::{Self, PriceState}, oracles::{Self, OraclePriceUpdate}};
    use sui::{clock::{Self, Clock}, sui::SUI, test_scenario::{Self, Scenario}};
    ...
    fun test_oracles_happy_pyth() {
        ...
        assert!(price_update.price().is_expo_negative() == false);
        assert!(
            price_update.metadata().pyth() == mock_pyth::get_price_obj<SUI>(&prices).get_price_info_from_price_info_object().get_price_feed(),
        );
